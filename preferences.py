"""Addon preferences — stores OpenAI API key, model, and temperature."""
...
class BlenderCopilotPreferences(AddonPreferences):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="OpenAI API Key", icon='KEYTYPE_KEYFRAME_VEC')
        ...
        if self.openai_api_key:
            ...
            box.label(text=f"\u2713 Key set ({len(key)} chars, starts with {key[:8]}\u2026)", icon='CHECKMARK')
            box.operator("aihouse.test_api_key", text="Test Connection", icon='PLUGIN')
        else:
            box.label(text="Copy your key then click 'Paste Key from Clipboard'", icon='INFO')
